...
interface SalesPipelineData {
  id: string;
  closeWon: string;
  // Add other sales properties as needed
}
...
const DashboardMetricsCards = () => {
  ...
  const [salesPipelineData, setSalesPipelineData] = useState<SalesPipelineData[]>([])
  ...
  const calculateProjectMetrics = () => {
    ...
    const calculatePercentage = (value: number) => Math.round((value / totalTasks) * 100) || 0
  ...
  const processFunnelData = (): FunnelDataItem[] => {
    ...
    const statusArray = Object.entries(statusCounts).map(([name, value]) => ({ name, value: value as number }))
  ...
  const FunnelTooltip = ({ active, payload }: FunnelTooltipProps) => {
